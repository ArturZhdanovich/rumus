...
class ModelToolExport extends Model {
	...
	function populateProductsWorksheet( &$worksheet, &$database, $languageId, &$priceFormat, &$boxFormat, &$weightFormat, &$textFormat )
	{
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('main_categories'),4)+1);
		...
		$this->setCell( $worksheet, $i, $j++, 'main_categories', $boxFormat );
		...
		foreach ($products as $row) {
			...
			$this->setCell( $worksheet, $i, $j, $mainCategories[$productId]['category_id'], $textFormat );
			}
			$j++;
