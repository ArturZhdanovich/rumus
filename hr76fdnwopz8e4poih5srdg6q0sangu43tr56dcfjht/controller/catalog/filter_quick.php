<?php
class ControllerCatalogFilterQuick extends Controller {
	private $error = array();  
	...
	public function index() {
		...
		if (!$this->config->get('config_filter_quick_edit')) {
			$this->redirect($this->url->link('catalog/filter', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/filter_quick', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	private function getListing($filter_group_id){
		$this->language->load('catalog/filter');
		$this->load->model('catalog/filter');
		...
		if ($this->user->hasPermission('modify', 'catalog/filter_quick')) {
			if(isset($this->request->get['filter_edit'])){		
				if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
					$this->model_catalog_filter->editFilter((int)$filter_group_id, $this->request->post);
				}
			}
		}
		...
		if(isset($this->request->get['filter_edit'])){		
			$this->template = 'catalog/quick_edit_filter/quick_filter_popup.tpl';
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/filter_quick', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['insert'] = $this->url->link('catalog/filter_quick/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link('catalog/filter_quick/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['link'] = $this->url->link('catalog/filter_quick&token=', 'token=' . $this->session->data['token'] . $url, 'SSL');
		
		if(isset($this->request->get['filter_edit'])){		
			return $this->getListing((int)$this->request->get['filter_group_id']);
		}
		...
		$this->data['sort_name'] = $this->url->link('catalog/filter_quick', 'token=' . $this->session->data['token'] . '&sort=fgd.name' . $url, 'SSL');
		$this->data['sort_sort_order'] = $this->url->link('catalog/filter_quick', 'token=' . $this->session->data['token'] . '&sort=fg.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/filter_quick', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'catalog/quick_edit_filter/quick_filter_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['filter_group_id'])) {
			$this->data['action'] = $this->url->link('catalog/filter_quick/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else { 
			$this->data['action'] = $this->url->link('catalog/filter_quick/update', 'token=' . $this->session->data['token'] . '&filter_group_id=' . $this->request->get['filter_group_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/filter_quick', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'catalog/quick_edit_filter/quick_filter_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
